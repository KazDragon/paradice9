...
{
    explicit impl(command_prompt &self) : self_{self}, edit_{munin::make_edit()}
    {
        buffer_.push_back("");
    }
    ...
    bool event(std::any const &ev)
    {
        ...
        if (vk_event != nullptr)
        {
            if (vk_event->key == terminalpp::vk::enter)
            {
                ...
                buffer_.push_back("");
                current_index_ = static_cast<int>(buffer_.size()) - 1;
                edit_->set_text("");
...
// CONSTRUCTOR
// ==========================================================================
command_prompt::command_prompt() : pimpl_{boost::make_unique<impl>(*this)}
{

...
/// auto stream = std::make_shared<odin::telnet::stream>(
///     underlying_stream, std::ref(io_service));
///
/// // Construct the various routers used by the options and the input_visitor.
/// auto command_router = std::make_shared<odin::telnet::command_router>();
/// auto negotiation_router = std::make_shared<odin::telnet::negotiation_router>();
/// auto subnegotiation_router = 
///     std::make_shared<odin::telnet::subnegotiation_router>();
///
/// // Also set up a callback for any plain text that comes through.
/// auto string_handler = [](auto const &text){my_string_handler(text);};
///
/// Construct an input_visitor to handle the input.
/// auto input_visitor = odin::telnet::input_visitor(
...
/// auto naws_client = std::make_shared<odin::telnet::options::naws_client>(

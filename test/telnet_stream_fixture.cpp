...
void telnet_stream_fixture::test_constructor()
{
    ...
    std::shared_ptr<odin::io::byte_stream> fake_stream(
...
void telnet_stream_fixture::test_sync_read_normal()
{
    ...
    // character.
    boost::asio::io_service     io_service;
    std::shared_ptr<fake_bytestream> fake_stream(
        new fake_bytestream(io_service));
    odin::telnet::stream       stream(fake_stream, io_service);

    fake_bytestream::input_storage_type data = { 'X' };

    fake_stream->write_data_to_read(data);

    input_storage_type actual_variant_array(stream.read(input_size_type(1)));

    std::string expected_result = "X";

    input_value_type expected_variant(expected_result);
    input_storage_type expected_variant_array = { expected_variant };

    CPPUNIT_ASSERT_EQUAL(
        expected_variant_array.size()
      , actual_variant_array.size());

    auto element0 = boost::get<std::string>(actual_variant_array[0]);
...
void telnet_stream_fixture::test_sync_read_normal_iac_iac()
{
    ...
    std::string expected_string("abcde\xFF");
...
void telnet_stream_fixture::test_sync_read_normal_iac_then_iac()
{
    ...
    expected_variant_array = { expected_variant };

    CPPUNIT_ASSERT_EQUAL(
        expected_variant_array.size()
      , actual_variant_array.size());

    element0 = boost::get<std::string>(actual_variant_array[0]);
...
void telnet_stream_fixture::test_sync_read_command()
{
    ...
    auto element0 = boost::get<odin::telnet::command>(actual_variant_array[0]);
...
void telnet_stream_fixture::test_sync_read_normal_command()
{
    ...
    fake_bytestream::input_storage_type data =
    {
        'a', odin::telnet::IAC, odin::telnet::NOP
    };
    ...
    auto element1 = boost::get<odin::telnet::command>(actual_variant_array[1]);
...
void telnet_stream_fixture::test_sync_read_partial()
{
...

    {
        ...
        auto actual_element0 =
            boost::get<std::string>(actual_variant_array[0]);

        CPPUNIT_ASSERT_EQUAL(expected_string0, actual_element0);

        auto actual_element1 =
          boost::get<odin::telnet::command>(actual_variant_array[1]);
    ...
    {
        std::string        expected_string2("bc");
        input_value_type   expected_variant2(expected_string2);
        input_storage_type expected_variant_array = { expected_variant2 };

        CPPUNIT_ASSERT_EQUAL(
            expected_variant_array.size()
          , actual_variant_array.size());

        auto actual_element2 =
            boost::get<std::string>(actual_variant_array[0]);
...
void telnet_stream_fixture::test_sync_read_many()
{
    ...
    std::string      expected_string0("abc");
    input_value_type expected_variant0(expected_string0);
    ...
    std::vector<odin::u8> subnegotiation3_array(
    ...
    std::string      expected_string4("def");
    ...
    auto element2 = boost::get<odin::telnet::command>(actual_variant_array[2]);

    CPPUNIT_ASSERT_EQUAL(element2, expected_command2);

    auto element3 =
        boost::get<odin::telnet::subnegotiation_type>(actual_variant_array[3]);
    ...
    auto element4 = boost::get<std::string>(actual_variant_array[4]);
...
void telnet_stream_fixture::test_async_read_normal()
{
    ...
    input_callback_type input_callback = [&](auto &&storage)
    {
        actual_variant_array = std::move(storage);
        ++called;
    };
...
void telnet_stream_fixture::test_available_iac()
{
    ...
    fake_bytestream::input_storage_type data = { odin::telnet::IAC };
...
void telnet_stream_fixture::test_available_during_async_callback()
{
    ...
    boost::optional<input_size_type> available;
    auto callback = [&](auto &&)
    {
...
void telnet_stream_fixture::test_sync_write_normal()
{
    ...
    std::string         element0("abc");
    output_value_type   variant0(element0);
    output_storage_type variant_array = { variant0 };
...
void telnet_stream_fixture::test_async_write_normal()
{
    ...
    std::function<void (output_size_type)> callback = [&](auto &&size)
    {
        ++called;
        output_size = size;
    };

...
private:
    ...
    void schedule_next_read(std::shared_ptr<paradice::connection> const &cnx)
    {
        ...
        cnx->async_read([this, wcnx = std::weak_ptr<paradice::connection>(cnx)](
                            serverpp::bytes) {
            ...
            if (std::shared_ptr<paradice::connection> cnx = wcnx.lock(); cnx)
            {
                if (cnx->is_alive())
                {
    ...
    void on_accept(serverpp::tcp_socket &&new_socket)
    {
        ...
        auto connection =
            std::make_shared<paradice::connection>(std::move(new_socket));
        ...
        connection->on_window_size_changed(
            [this, wcnx = std::weak_ptr<paradice::connection>(connection)](
                auto w, auto h) { this->on_window_size_changed(wcnx, w, h); });

        connection->async_get_terminal_type(
            [this, wcnx = std::weak_ptr<paradice::connection>(connection)](
                auto const &type) { this->on_terminal_type(wcnx, type); });
    ...
    void on_terminal_type(
        std::weak_ptr<paradice::connection> const &weak_connection,
        std::string const &terminal_type)
    {
        ...
        if (auto cnx = weak_connection.lock(); cnx)
        {
            ...
            std::unique_lock<std::mutex> pending_size_lock(
                pending_sizes_mutex_);
            ...
            auto client = std::make_shared<paradice::client>(
                io_context_,
                context_,
                std::move(*cnx),

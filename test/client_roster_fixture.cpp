...
TEST(a_client, includes_admin_in_help_for_a_caller_with_admin_access)
{
    boost::asio::io_context io_context;
    fake_context context;
    auto channel = std::make_shared<fake_channel>();
...
TEST(a_client, lists_admin_access_commands_for_help_admin)
{
    boost::asio::io_context io_context;
    fake_context context;
    auto channel = std::make_shared<fake_channel>();
    ...
    ASSERT_EQ(0u, context.room_messages.size());
    ASSERT_EQ(
        "Admin commands:\n/admin list_accounts\n/admin list_characters <account>"_ts,
        context.direct_messages[0]);
}

TEST(a_client, includes_shutdown_in_help_admin_for_a_caller_with_admin_shutdown)
{
    boost::asio::io_context io_context;
    fake_context context;
    auto channel = std::make_shared<fake_channel>();
    ...
    includes_set_password_in_help_admin_for_a_caller_with_admin_set_password)
{
    ...
    ASSERT_EQ(0u, context.room_messages.size());
    ASSERT_EQ(
        "Admin commands:\n/admin list_accounts\n/admin list_characters <account>\n/admin set_password <account> <password>"_ts,
        context.direct_messages[0]);
}
...
TEST(a_client, reports_help_admin_as_unknown_without_admin_access)
{
    boost::asio::io_context io_context;
    fake_context context;
    auto channel = std::make_shared<fake_channel>();
    ...
    ASSERT_EQ(1u, context.direct_messages.size());
    ASSERT_EQ(0u, context.room_messages.size());
    ASSERT_EQ("Unknown command: /help admin"_ts, context.direct_messages[0]);
}
